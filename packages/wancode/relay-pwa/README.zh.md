...
- `createPwaRelayController` 会注册 PWA 设备、签发短期令牌并拨 Relay。公网
  ...
  `createWebCryptoDeviceIdentity` 生成。握手签名走 `createWebCryptoSignedHandshakeEnvelope`。follow-up 密封走 `createWebCryptoSealedRelayEnvelope`。关闭后的会话在 `reconnect` 之前拒绝 send 和 drain。
