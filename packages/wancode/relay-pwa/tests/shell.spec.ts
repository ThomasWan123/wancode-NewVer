...
describe('PWA installable shell', () => {
  ...
  it('deletes stale shell caches and refuses credentialed cache names', () => {
    ...
    expect(decidePwaCacheRetention('wancode-pwa-shell-v13')).toBe('delete')
    expect(decidePwaCacheRetention('wancode-pwa-shell-v14')).toBe('delete')
    expect(decidePwaCacheRetention('wancode-pwa-shell-v15')).toBe('delete')
  ...
  it('emits static shell files that match the manifest and never embed secrets', () => {
    ...
    expect(files['index.html']).toContain('<title>WanCodeNewVer</title>')
    expect(files['index.html']).toContain('name="origin"')
    expect(files['index.html']).toContain('name="pair"')
    expect(files['index.html']).toContain('name="session"')
    expect(files['index.html']).not.toContain('name="session" required')
    expect(files['pair.js']).toContain("if (trimmed === '') return 'queue'")
    expect(files['index.html']).toContain('name="follow"')
    expect(files['index.html']).toContain('id="follow"')
    ...
    expect(files['pair.js']).toContain('v1:box:')
    expect(files['pair.js']).toContain('destinationDeviceId')
    expect(files['pair.js']).not.toContain("sessionStorage.setItem('follow'")
    ...
    expect(files['pair.js']).toContain('/v1/devices/revoke')
    expect(files['pair.js']).toContain('dropPairedDevice')
    expect(files['pair.js']).not.toContain("sessionStorage.setItem('wancode-relay-accessToken'")
