...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`WanCodeNewVer ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart WanCodeNewVer after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("WanCodeNewVer {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart WanCodeNewVer after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(WanCodeNewVer !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart WanCodeNewVer after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "WanCodeNewVer" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
