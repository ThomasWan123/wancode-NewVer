...
Choosing **Download** first rechecks that the advertised version is unchanged, persists a rollback transition, then requests the immutable Wancode asset for that GitHub release. Wancode streams at most 1 GiB into a private versioned user-data directory and rejects an incomplete DMG or Windows PE. Windows updates additionally require a trusted Authenticode signature before the NSIS installer can be opened. After the target version starts, the tray offers a confirmation-gated rollback that re-downloads and revalidates the preceding version; rollback completion clears the transition. **Restart and Install** requests orderly Cordis teardown before the current process exits.
