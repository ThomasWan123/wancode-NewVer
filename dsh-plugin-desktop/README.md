...
The Electron executable is minimal bootstrap code. It acquires the single-instance lock, resolves the selected DSH profile, provides the native runtime capability, and boots the Host Cordis root in the Electron main process. The `desktop-shell` Host plugin owns the `BrowserWindow`, navigation policy, settings namespace, and close-versus-quit lifecycle through Cordis effects. The native runtime owns the physical tray, including **Open Diagnostics Folder**. `desktop-shell`, `desktop-profiles`, `desktop-terminal`, `desktop-updates`, and `desktop-relay` contribute additional effect-scoped commands through its ordered item registry. `desktop-relay` is disabled by default, never listens, and only dials an outbound WSS URL after an explicit connect. When enabled, it derives an HTTPS control origin from that URL so register, token mint, device list, pairing-grant mint, and revoke stay outbound HTTP. After connect, pairing-grant mint may use the session token instead of an OIDC assertion. When `desktopRuntime` is present, apply registers effect-scoped **Connect Relay** and **Copy Pairing Code** tray commands. Connect Relay enrolls on loopback without OIDC, dials, then applies queued PWA mail. Pairing-grant mint may use the session token. Copy Pairing Code copies the display form and notifies without including the code. Public hosts fail closed. Missing runtime stays idle; `inject` stays empty. The local device identity is generated once into Windows Credential Manager; enroll and handshake use the public keys only. Sealed PWA follow-ups are opened with `openSealed` without copying private keys onto the identity object. Progress events are sealed to a PWA with `sealTo`. `sealDesktopRelaySessionEvent` allows only compact progress types and never seals prompt text. Empty recipient encryption keys fail closed. `sendDesktopRelaySessionEvent` pushes that box over the outbound socket. After `connect`, `sendProgress` uses the same path and refuses to send before the socket exists. `sendPresence` seals online/offline frames the same way. `drainDesktopRelayMail` reclaims queued boxes, opens them, and acks only queued ids. `processDesktopRelayMail` applies follow-ups locally first and acks queued ids only after that sink succeeds. `createDesktopRelayFollowUpSink` refuses missing session ids. Session id `queue` creates a new Host session through `sessions.create()` then `prompt(..., 'queue')` without injecting Host services; missing `create` still fails closed. Approval and cancel sinks refuse missing request ids. `createDesktopRelayApplySinks` builds those three lookups together. `createDesktopRelayHostApplySinks` queues follow-ups through Host `prompt([{ type: 'text', text }], 'queue')` and maps approvals to Host `respond('allowed-once' | 'rejected')` without injecting Host services. `prepareDesktopRelay` may take those sinks so `processMail` can apply follow-ups after connect. `lookupDesktopRelayHostApplySinks` probes `ctx.get('sessions')` without adding a required inject. Host `prompt` / `respond` and Client `submit` / `decide` shapes are both accepted. `bindDesktopRelay` returns that handle so connect and processMail can run after apply, and disposes it with the Host effect. `openDesktopRelaySession` enrolls the stored identity, mints a token, and dials without exposing private keys. A missing handshake nonce is minted with WebCrypto. `openDesktopRelayMailbox` then applies queued PWA mail. `processMail` repeats that lookup so a late Host session still applies follow-ups. `applyDesktopRelayPayloads` hands follow-ups to a local session sink so model credentials stay on the desktop. Approval and cancel frames require those sinks; they are not dropped.
...
The launcher reads the same file resolved by the active `@deepseek-ai/dsh-settings-file` row before composing a generation. The Host registers the `dsh-desktop` namespace with the standard settings service. Both presentation mode and update channel apply after one orderly restart.
...
New desktop sessions default to the `read-only` permission preset: sandbox mode `read-only` and approval policy `ask`. Set `DSH_PERMISSION_MODE` to `workspace-write` or `danger-full-access`, or choose another preset in the Permissions selector, to widen later sessions. An unknown `DSH_PERMISSION_MODE` fails at profile composition.
...
Packaged macOS and Windows applications query the selected stable or beta release stream from `ThomasWan123/wancode-NewVer` on GitHub 60 seconds after startup and every six hours after a completed check. Each no-cache request has a 15-second deadline and shares one in-flight operation with the **Check for Updates…** tray command. Only canonical `v<semver>` tags matching GitHub's release metadata are accepted. Background network, HTTP, timeout, invalid-response, equal-version, and older-version outcomes are silent. A manual check always opens a native result dialog. Development, unpackaged, and Linux launches do not download an installer.

Choosing **Download** first rechecks that the advertised version is unchanged, persists a rollback transition, then requests the immutable Wancode asset for that GitHub release. Wancode streams at most 1 GiB into a private versioned user-data directory and rejects an incomplete DMG or Windows PE. Windows updates additionally require a trusted Authenticode signature before the NSIS installer can be opened. The target version must report terminal Host and renderer health within 30 seconds. A failed startup or missed deadline makes one loop-protected Windows recovery attempt: the preceding installer is downloaded and validated again, then launched without another confirmation dialog. A healthy start retains the confirmation-gated tray rollback, failed automatic downloads remain manually retryable there, and starting the preceding version clears the transition. **Restart and Install** requests orderly Cordis teardown before the current process exits.
...
Closing the window hides it while the Host Cordis tree continues running. The tray reopens the window, opens the diagnostics folder, selects the active profile, opens the isolated DSH terminal, checks for a stable release, changes mode through the standard settings namespace, or requests an explicit quit. A renderer crash offers reload, diagnostics, or restart without tearing down the Host first. Profile and mode changes both dispose the current Cordis tree before Electron relaunches. Native quit, `SIGINT`, and `SIGTERM` also request disposal before exit; a five-second deadline or a repeated request forces the final exit. Navigation and redirects remain on the exact loopback origin; external HTTP, HTTPS, and mail links open in the operating system, while the renderer uses `contextIsolation`, the Chromium sandbox, and no Node integration. Packaged launches also mirror stdout and stderr into `logs/wancode.log` under Electron user data so a GUI process still has a local diagnostics file.
...
The signed release gate must run on a disposable Windows runner because installer registry state is modified. Set `WANCODE_WINDOWS_LIFECYCLE_DISPOSABLE=1`, `WANCODE_PREVIOUS_WINDOWS_INSTALLER`, and `WANCODE_PREVIOUS_WINDOWS_VERSION` to an older trusted release. `WANCODE_WINDOWS_TRUSTED_THUMBPRINTS` must list the approved SHA-1 certificate thumbprints (comma-separated, including both certificates during an intentional rotation). The gate verifies the previous installer, current installer, installed application, and uninstaller against the approved certificate set while exercising install, upgrade, rollback, current-version restore, and uninstall. It fails closed, attempts cleanup on every interrupted transition, and rejects residual install directories, uninstall registry values, and standard shortcuts.
...
- Windows update handoff validates PE structure and Authenticode trust. Publisher pinning is enforced by the signed-release gate; runtime publisher pinning and SmartScreen reputation remain release work.
