...
describe('desktop terminal environment', () => {
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(welcome).toContain('Wancode NewVer 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart Wancode NewVer after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"Wancode NewVer {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "Wancode NewVer" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
