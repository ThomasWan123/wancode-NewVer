...
describe('published package surface', () => {
  ...
  it('exposes the Host plugin and desktop-owned client face', () => {
    ...
    expect(manifest.exports).toHaveProperty('./credentials-win', {
      types: './lib/types/credentials-win.d.ts',
      default: './lib/credentials-win.js',
    })
    ...
    expect(manifest.exports).toHaveProperty('./relay', {
    ...
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/relay')
  })
  ...
  it('builds public Host plugins and their private native bootstraps', () => {
    ...
    expect(config).toContain("'credentials-win': 'src/credentials-win.ts'")
    ...
    expect(config).toContain("relay: 'src/relay.ts'")
  })
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    expect(manifest.scripts?.['check:win-package']).toContain('tests/desktop-log.spec.ts')
    expect(manifest.scripts?.['check:win-package']).toContain('tests/crash-recovery.spec.ts')
