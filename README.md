...
| 移动 PWA（M3） | **进行中**：会话快照、设备选择、密封控制、通知、重连 drain、可安装 Manifest 契约；模型凭据留在桌面；HTML/SW 尚未交付 |
...
Windows 聚焦门禁覆盖 252 项桌面测试，外加完整运行时依赖闭包。尚未交付的能力会写在路线图中，不会写成已经可用。
...
3. **移动 PWA** — 进行中：会话快照、密封控制、可安装 Manifest 契约；HTML / service worker 尚未交付。
