...
| Mobile PWA (M3) | **In progress**: session snapshots, device selection, sealed control, notifications, reconnect drain, and an installable manifest contract; model credentials stay on the desktop; HTML/SW not shipped yet |
...
3. **Mobile PWA** — in progress: session snapshots, sealed control, and an installable manifest contract; the HTML/service-worker runtime is not delivered yet.
